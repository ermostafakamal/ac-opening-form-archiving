...
export class LoginService {
  ...
  apiUrl = `https://dmsreportapi.bankasia-bd.com/api/Login/IsAuthentic?username=Admin&password=Asdf%401234`;
  ...
  checkCredentials(userId:string, password: string ) { 
    const loginApiUrl = `https://dmsreportapi.bankasia-bd.com/api/Login/IsAuthentic?username=${userId}&password=${password}`;

    this._http.get(loginApiUrl).subscribe((res:any)=>{
      if(res){
        this.isUserLoggedIn.next(true);
        localStorage.setItem('userId', userId);
        localStorage.setItem('IsAuthenticated', JSON.stringify(true));
        sessionStorage.setItem('IsAuthenticated', JSON.stringify(true));
        localStorage.setItem('logedCustName', userId);
        localStorage.setItem('logedCustEmail', userId);
        //window.location.href = "https://enadocreport.com/adminreport/";
        window.location.href = "http://dmsreporthome.bankasia-bd.com/";
      }          
  ...
  login(userId:string, password: string): void {
    ...
    //const loginApiUrl = `https://bgreport.bgc-bd.com/api/Login/IsAuthentic?username=${userId}&password=${password}`;
    const loginApiUrl = `https://localhost:7213/api/Login/IsAuthentic?username=${userId}&password=${password}`;
